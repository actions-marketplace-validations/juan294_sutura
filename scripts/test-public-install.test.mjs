...
test('public install uses only sutura@0.3.0 and verifies the independently resolved release SHA', async () => {
  ...
  try {
    ...
    const result = await verifyInstall({
      mode: 'public', root: temporary, releaseVersion: '0.3.0',
      ...
      dependencies: {
        ...
        pack: async (source, destination) => {
          ...
          const tarball = join(destination, 'sutura-0.3.0.tgz');
        ...
        install: async (_tarball, consumer) => {
          ...
          await writeFile(join(consumer, 'node_modules', 'sutura', 'package.json'), JSON.stringify({
            name: 'sutura', version: '0.3.0', dependencies: {},
          }));
        ...
        invoke: async (_binary, args, consumer, environment) => {
          ...
          if (args[0] === '--version') return '0.3.0\n';
    ...
    assert.deepEqual(calls[0], ['resolve', '0.3.0']);
    assert.deepEqual(calls[1], ['pack', 'sutura@0.3.0']);
...
test('public wrapper accepts one explicit exact semver and rejects mutable or ranged releases', async () => {
  assert.equal(parseReleaseVersion([]), '0.3.0');
