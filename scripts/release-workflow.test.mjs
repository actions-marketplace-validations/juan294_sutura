...
test('all release-bearing packages and the public API declare 0.3.0', async () => {
  ...
  assert.deepEqual(manifests.map(({ version }) => version), Array(7).fill('0.3.0'));
  assert.match(await text('packages/core/src/index.ts'), /VERSION = '0\.3\.0'/u);
});
...
test('versioned release evidence requirements name every authorization gate', async () => {
  const requirements = JSON.parse(await text('docs/demo/sutura-v0.3.0-release-evidence-requirements.json'));
  assert.equal(requirements.releaseVersion, '0.3.0');
