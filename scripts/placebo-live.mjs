...
export async function runRemoteCase({ controllerSha, subjectSha, caseId, skipGate = false, counterfactual = false,
  ...
  try {
    ...
    assertPublicArtifactSafe(artifact, [process.env.NEBIUS_API_KEY, process.env.TAVILY_API_KEY, process.env.OPENAI_API_KEY, process.env.TYPESAFE_API_KEY, process.env.CONTREE_TOKEN]);
...
async function artifactCommand(args) {
  ...
  const secrets = [process.env.NEBIUS_API_KEY, process.env.TAVILY_API_KEY, process.env.OPENAI_API_KEY, process.env.TYPESAFE_API_KEY, process.env.CONTREE_TOKEN];
