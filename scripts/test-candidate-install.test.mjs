...
test('candidate pack accepts npm 11 and npm 12 JSON output only when one package is present', () => {
  assert.equal(packedFilename('[{"filename":"sutura-0.3.1.tgz"}]'), 'sutura-0.3.1.tgz');
  assert.equal(packedFilename('{"filename":"sutura-0.3.1.tgz"}'), 'sutura-0.3.1.tgz');
  assert.equal(packedFilename('{"sutura":{"filename":"sutura-0.3.1.tgz"}}'),
    'sutura-0.3.1.tgz');
...
test('candidate install uses the local tarball and exact candidate Action SHA without network', async () => {
  ...
  try {
    ...
    assert.equal(result.packageVersion, '0.3.1');
    ...
    assert.ok(!JSON.stringify(calls).includes('sutura@0.3.1'));
  } finally {
