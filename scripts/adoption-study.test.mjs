...
test('public release verification binds npm metadata and immutable tag to one candidate', async () => {
  const dependencies = {
    ...
    fetch: async () => new Response(JSON.stringify({
      name: 'sutura', version: '0.3.0', dist: { integrity: 'sha512-public' },
    }), { status: 200, headers: { 'content-type': 'application/json' } }),
  };
  await assert.doesNotReject(() => verifyPublicRelease('0.3.0', SHA, dependencies));
  await assert.rejects(() => verifyPublicRelease('0.3.0', 'b'.repeat(40), dependencies), /differs/u);
});
...
test('finalize requires three distinct unfamiliar repositories, both language families, and all outcomes', async () => {
  ...
  try {
    ...
    assert.equal(first.packageVersion, '0.3.0');
