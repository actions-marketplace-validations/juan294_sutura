...
import { ContreeExecutor, OpenAiClient, TypeSafeClient, createTokenFactoryClient, executeExternalVerification, readTrustedPolicyAtCommit, snapshotCleanSourceAt, type Config, type ExternalVerificationResult, } from '@sutura/core';
...
async function execute(inputs: ActionVerifyInputs, context: ActionVerificationContext): Promise<ActionVerificationOutcome> {
  ...
  try {
    ...
    const typesafeAudit = config.typesafeApiKey ? new TypeSafeClient({ apiKey: config.typesafeApiKey, ledger: llm.ledger }) : undefined;
    const executor = new ContreeExecutor({ token: config.contreeToken, project: config.contreeProject, maxOps: config.maxOps });
    const result = await executeExternalVerification({ mode: 'live', request: validated.request, policy: trusted.policy, executor, llm, ...(secondOpinion ? { secondOpinion } : {}), ...(typesafeAudit ? { typesafeAudit } : {}), sourceDir: snapshot.dir, snapshotSha256: snapshot.snapshotSha256, policySha256: trusted.sha === 'default' ? createHash('sha256').update(JSON.stringify(trusted.policy)).digest('hex') : trusted.sha, budgets: config.repairBudgets });
