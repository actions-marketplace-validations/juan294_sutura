...
describe('resolveActionCommit', () => {
  it('resolves a lightweight release tag to one exact commit', async () => {
    const run = vi.fn().mockResolvedValue(`${COMMIT}\trefs/tags/v0.3.0\n`);

    await expect(resolveActionCommit({ version: '0.3.0', cwd: '/tmp/repo', run }))
  ...
  it('accepts only an exact explicit candidate commit without a network command', async () => {
    ...
    await expect(resolveActionCommit({
      version: '0.3.0', cwd: '/tmp/repo', run, explicitCommit: COMMIT.toUpperCase(),
    })).resolves.toBe(COMMIT);
    expect(run).not.toHaveBeenCalled();

    await expect(resolveActionCommit({
      version: '0.3.0', cwd: '/tmp/repo', run, explicitCommit: 'main',
    })).rejects.toBeInstanceOf(ReleaseResolutionError);
  });

  it.each(['', `${COMMIT}\trefs/tags/v0.3.0\n${TAG_OBJECT}\trefs/tags/v0.3.0\n`, 'not-a-sha\trefs/tags/v0.3.0\n'])(
    'fails closed for missing, ambiguous, or malformed tag output',
    async (output) => {
      await expect(resolveActionCommit({
        version: '0.3.0', cwd: '/tmp/repo', run: vi.fn().mockResolvedValue(output),
      })).rejects.toBeInstanceOf(ReleaseResolutionError);
    },
  );
});
